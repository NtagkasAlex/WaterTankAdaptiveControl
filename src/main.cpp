...
void loop() {
  ...
  //adaptation laws for parameters
  ...
  float u = (-a1_dot * f(h1) + a1_hat * a1_hat * g + a2_dot * f(h2) + a1_hat * a2_hat * f(h1) * f_dot(h2) + g * a2_hat * a2_hat + v - k1 * psi1 - k2 * psi2) / (a1_hat * b_hat * f_dot(h1));

  //we store u because we use it at B matrix
  store_u = u;
  ...
  //finally we create the dynamics for the reference model
  float xm1_dot = xm2;
